...
if test "X$1" = Xinstall ; then
  ...
  if ! test -f zephyr-sdk-${VER}_linux-x86_64.tar.xz ; then
    wget      https://github.com/zephyrproject-rtos/sdk-ng/releases/download/v$VER/zephyr-sdk-${VER}_linux-x86_64_gnu.tar.xz
  ...
  if ! test -d zephyr-sdk-${VER} ; then
    tar xJf zephyr-sdk-${VER}_linux-x86_64_gnu.tar.xz
  fi
